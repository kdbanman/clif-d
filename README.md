...
- [ ] **Keep high-level requirement status in sync.** Today, low-level requirements get their `status` updated through the `plan-requirement` -> `implement-plan` cycle, but the high-level requirements they realize do not. After REQ-029 (low-level, "Structural validation on every PRD load") shipped, REQ-025 (high-level NFR, "Defense-in-depth PRD structural validation on access") still read `not_started`. Update the `plan-requirement` skill so that when it builds a plan, it considers which high-level requirements that plan will fully or partially realize, and adds explicit status-update tasks for those high-level requirements to the plan document. The implementer then closes them out as part of the same cycle that closes the low-level requirements.
- [ ] **Make `status` a mandatory PRD requirement field.** Today `status` is optional and an absent value is treated as `not_started`. That convention has produced confusion: a high-level requirement that was never explicitly started reads identically to one that has never been touched. Update the PRD schema to require `status` on every requirement (with `not_started` as a value the author must write down), and update create-initial-prd, plan-requirement, and implement-plan to set it explicitly. This also reduces the surface area of the per-skill workaround above (high-level status sync), at the cost of a one-time migration of existing PRDs.
- [ ] **`compactify-artifacts` should rewrite re-routed lessons in the destination's voice.** When a candidate lesson is rerouted from `lessons_learned/` into a design document (`backpressure.md`, `architecture.md`, `dev-environment.md`) rather than into `lessons.md`, it should be rewritten in that document's voice and at that document's level of generality, not pasted in verbatim. A lesson that reads "during REQ-027 the jscpd `--threshold` was misread as a reporting threshold" belongs in `backpressure.md` as a paragraph about why `threshold: 0` is the load-bearing setting, not as a dated incident note. Add this expectation to the destination-selection step of the skill so re-routed material reads like part of its new home rather than like a transplant.
