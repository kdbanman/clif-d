...
- [ ] **Make sure git hooks end up in cloud environments.**  Because git hooks are not committed to version control (right?) we need to find a way for the boot strap environment script to idempotently ensure that the git hook-based back pressure gates are in place.  Including when changes are made to the back pressure system. The design back pressure skill, and the bootstrap environment skill may not be run after the project is initialized. But there still needs to be a mechanism to make sure that the hooks are propagated out to all dev environments.
