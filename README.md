...
| `plan-requirement` | Takes PRD requirement IDs, resolves the full dependency graph, explores the codebase, and produces a self-contained implementation plan (Markdown) with TDD step ordering | After `bootstrap-dev-environment` and each prior `implement-plan`; before `implement-plan` | `clif-d/plans/active/plan-REQ-NNN.md` | `clif-d req show`, `clif-d req dep`, `clif-d req next` |
...
- **Add `clif-d req ref add/rm <REQ-ID> <ITEM-ID>`** (or similar) -- idempotent single-reference edits on `architecture_refs` and `context_refs`, eliminating the read-then-write dance and the replace-semantics gotcha that `create-architecture`, `design-backpressure`, and `bootstrap-dev-environment` all currently have to document around `clif-d req edit`. A bulk variant (e.g. `clif-d req ref add <CTX-ID> --all` to apply a CTX to every requirement) would also kill the explicit loop the backpressure and dev-environment skills run over "typically all of them".
- **Add `clif-d ctx ensure` / `clif-d arch ensure`** (idempotent upsert keyed by `reference_link`) -- `design-backpressure` and `bootstrap-dev-environment` both need to check "does a constraint item pointing at `clif-d/backpressure.md` / `clif-d/dev-environment.md` already exist?" before adding, and currently do it by grepping `ctx ls` output on fragile title substrings. An atomic upsert keyed by `reference_link` would replace that check with one deterministic call.
- **Add `clif-d plan archive <REQ-ID> --commit=<sha>`** -- the last raw file operation in `implement-plan` is a shell `mv clif-d/plans/active/plan-REQ-NNN.md clif-d/plans/executed/` plus a manual append of `**Implementation commit:** <sha>` to the plan header. A single subcommand would do both atomically and validate that the plan header has the expected shape first.
