...
- [ ] **Nuance the `implement-plan` Red-Green-Refactor rule.** The skill's default rule is "fix the fixture, not the validator" when a test fails during refactor. That rule is right when the failing test represents a regression in a *contract* (function signature, output format, rule semantics held constant). It is wrong when the failing test represents a regression in the *validator itself* -- e.g., a lint rule was silently relaxed, a schema constraint was weakened, or a gate was disabled. During REQ-027 a failing backpressure-lint fixture would have been "fixed" by rewriting the fixture if the rule were followed literally, masking the real regression: the ESLint override that should have been catching it. Add a triage question to the Red-Green-Refactor section: "Did the expectation change, or did the enforcer weaken?" The first answer means fix the fixture; the second means fix the validator.
- [ ] **Improve the `compactify-artifacts` skill.** Running it once against a large accumulated backlog (8 executed plans, 8 lessons files) surfaced several gaps:
  - **Scope-per-run.** The skill today assumes the user wants to compact everything in `plans/executed/` and `plans/lessons_learned/` in one pass. For large backlogs this is too much to interrogate well. Instead, the skill should first survey the backlog and propose a *reasonable chunk* (e.g., a logically grouped set of related plans, or a bounded number of lessons) to process in the current run, leaving the rest for a future run. The user confirms the chunk before interrogation begins.
  - **Implementation commit SHA and post-compaction deletion, explicit in the skill.** Every archive entry must record the implementation commit SHA of its plan (verified via `git cat-file -e <sha>` and cross-checked against `prd.json`'s `implementation_commit` for each requirement). After archive entries and durable lessons are written, the raw executed-plan files and lessons-learned files are deleted -- git history is the long-term record. The current skill text covers both points but could be more explicit that the SHA is the load-bearing linkage between the compact archive and the deep-dive source material.
  - **Be explicit about where results go.** Today the skill implies everything becomes either a `lessons.md` entry or a discard. In practice, most candidate lessons belong elsewhere: a new paragraph in `backpressure.md` (for guardrail-adjacent lessons), a clarification in `architecture.md` (for structural lessons), a new PRD requirement (for foot-guns the tool itself should eliminate), or -- speculation -- a `.claude/rules/` glob-matched rules file so agents stumble into the lesson naturally when editing the relevant files, rather than relying on a global `lessons.md` they may never read. The skill should list these destinations in the interrogation step and default to `lessons.md` only when no better home exists.
  - **Per-candidate durability brief.** Raw lessons-learned files have a very low signal-to-noise ratio. Most entries are one-off typos, library-API confusion, or transient tooling gotchas already captured by existing precedent in the codebase. The skill should not surface every candidate to the user with a bare "keep / absorb / discard?" question. For each candidate worth surfacing, the skill should first do the homework: read the code, check whether precedent already exists, check whether the design docs already capture it. Then present the candidate as a short decision brief -- lesson verbatim, concrete codebase context, a durability argument, a non-durability argument, and action options that each carry their own rationale. Candidates that fail the durability bar on the skill's own investigation should be auto-discarded without bothering the user.
