...
- **Determine design for bulk reference backfill** (e.g. `clif-d req ref add <CTX-ID> --all` or similar) so it can be added as a low-level requirement under REQ-033. The `design-backpressure` and `bootstrap-dev-environment` skills both apply a single CTX to "every requirement that will be implemented in this environment (typically all of them)", and the single-element `clif-d req ref add` (REQ-034) leaves that loop explicit in skill prose. A bulk variant would kill the loop, but the shape is not yet clear. Open questions to resolve before specifying:
  - What does "all" mean -- every requirement in the PRD, or some filter (status, abstraction_level, priority, a predicate expression)?
  - How should the filter be expressed on the CLI -- boolean flags (`--status=...`), a match expression, literal IDs, or an existing `req ls`-style selector reused here?
  - What are the semantics when some targets already contain the ref (presumably the same idempotency as the single-element variant) vs when some fail validation (all-or-nothing atomicity, or partial success with a per-target report)?
  - Are there use cases beyond the two current skills (backpressure, dev-environment) that would constrain the shape -- e.g. `extend-low-level-requirements` or `create-architecture` applying a ref to a discovered subset?

  Once these are answered, promote to a low-level requirement under REQ-033.
