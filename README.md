...
- **Enrich skill table with pipeline position, outputs, and CLI commands** -- the Skills table in the README lists each skill's purpose but omits the operational detail an agent or developer needs at a glance. Update each row to include: (1) where the skill sits in the pipeline (what it consumes and what feeds it), (2) the artifact files it generates under `clif-d/`, and (3) up to three relevant `clif-d` CLI commands. Also update each skill's frontmatter `description` field to include the same operational detail -- the description is what the agent sees first when selecting a skill, so it should be at least as informative as the README row. Hard limit: 1024 characters (enforced by the Claude Code skill system).
