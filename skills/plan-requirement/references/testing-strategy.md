...
**What actually matters:** Fowler's 2021 article cuts through the debate. His biggest objection to the whole discussion is that people love debating proportions when they should focus on writing tests that establish clear boundaries, run quickly and reliably, and only fail for useful reasons. His second-biggest objection is that the terms "unit test" and "integration test" mean different things to different people, making the debate partly definitional.
...
Not all code deserves equal testing investment. Before choosing test types and proportions, assess your codebase by asking:
...
- **Small tests** run in a single process, on a single thread, and must not perform I/O (no disk, no network, no databases). They are fast and deterministic.
