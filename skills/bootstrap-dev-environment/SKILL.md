...
11. **Backfill PRD references.** The dev environment is a shared constraint that affects all implementation. Update `clif-d/prd.json` via the CLI -- do not hand-edit:

    a. Check whether a dev-environment context item already exists:

        ```
        clif-d ctx ls clif-d/prd.json | grep -i "dev environment"
        ```

       If one is present, record its `CTX-NNN` ID and skip step 11b. Also confirm (in the same listing) that the backpressure context item added by `design-backpressure` is still present -- if it is missing, stop and return to `design-backpressure`; do not re-create it here.

    b. If no dev-environment context item exists, create one with `clif-d ctx add`. Set `type` to `constraint` and describe the bootstrap command and containerization choice:

        ```
        cat <<'EOF' > /tmp/ctx-dev-env.json
        {
          "title": "Development environment bootstrap",
          "description": "<copy or summarize the bootstrap command, containerization choice, and reproducibility guarantees from clif-d/dev-environment.md>",
          "type": "constraint",
       ...
       The CLI echoes the added item with its assigned `CTX-NNN` ID to stdout. Record it as `$CTX_ID` for step 11c.

    c. For every requirement that will be implemented inside this environment (typically all of them), add `$CTX_ID` to the requirement's `context_refs` via a read-then-write pattern:
       ...
       **Gotcha:** `clif-d req edit` uses replace semantics -- the `context_refs` array in the stdin JSON replaces the existing array wholesale, it does not merge. You MUST read the current value first and send the full merged array. Omitting a ref that was already present (including the backpressure CTX added by `design-backpressure`) will delete it.

    d. Run `clif-d validate clif-d/prd.json`. Exit 0 confirms the new context item and all backfilled refs resolve. Fix any reported issues before handoff.
