...
1. **Generate the design document** at `clif-d/backpressure.md` in the product repository, following the structure above. Include the practitioner-facing content (how to run checks manually, how to handle failures, suppression prohibition summary) directly in the backpressure document. The backpressure document is both the design record and the developer reference. Create the `clif-d/` directory if it does not yet exist. Reference the bootstrap command by the name `bootstrap-dev-environment` will wire -- do not assert that hooks are installed yet.
