...
Treat this as a hard engineering rule, not a moral stance. Suppressions should be avoided *wherever* possible -- but real codebases sometimes inherit dependencies or generated code that force non-compliance with a rule you'd otherwise want enforced. The policy accommodates that reality precisely, and no more.
...
**Directory-scoped suppressions in tool configuration are the permitted escape hatch.** If a specific directory genuinely cannot comply with a rule -- a vendored third-party library, a generated-code directory, a migration-era legacy tree that will be cleaned up on a schedule -- scope the rule's configuration to exclude that directory (e.g. `overrides` in ESLint, `exclude` in `tsconfig.json`, per-path `ignore` in ruff). Record the scope and the reason in the backpressure document's Relaxations section so it is a public, reviewed trade-off rather than a quiet bypass. Directory scope is the *most specific* scope allowed; file-level and function-level suppressions remain prohibited even in configuration.

If a rule is wrong for the codebase as a whole, **change the rule globally** with a documented rationale in the Relaxations section and disable it in the tool's configuration. Global relaxations are a small, visible list a reviewer (human or agent) can audit; line-level suppressions are not.

This rule is not just stated -- it is **enforced by meta-backpressure**: a pre-commit check that scans staged changes for any line-level or function-level suppression directive in any supported format and fails the commit if one is introduced. A rule without enforcement is a suggestion, and suggestions are not backpressure (see "Hard gates, not suggestions" above). The scanner does not flag directory-scoped configuration changes, because those are the sanctioned mechanism.
...
**This table is the only legitimate place to turn a rule off globally, and the only place to record a directory-scoped exclusion.** Line-level and function-level suppressions are forbidden (see §5). If a rule must be disabled for the whole project, or scoped out of a specific directory (e.g. vendored code, generated output), record it here with a written rationale and implement it in the tool's configuration file. Every entry is a public, reviewed trade-off -- not a silent bypass.
...
>
> **Directory-scoped suppressions in tool configuration are the permitted escape hatch.** When a specific directory genuinely cannot comply with a rule (vendored dependencies, generated code, a time-boxed legacy tree), exclude that directory from the rule in the tool's configuration file and record the scope and reason in §4 "Relaxations from Maximum Strictness" above. Directory scope is the most specific scope allowed; file- and function-level suppressions are never the answer.
>
...
>
> This policy is enforced by a meta-backpressure pre-commit check (see §6 "Hook Architecture") that scans staged changes for any line-level or function-level suppression directive and fails the commit if one is introduced. The rule is not a convention to remember -- it is a gate. Directory-scoped configuration changes are not flagged by the scanner; they are the sanctioned mechanism.

If the project has narrow, legitimate reasons a specific rule cannot apply in a specific location, the answer is always to **scope the rule's configuration to exclude that directory** in §4. Line-level and function-level suppressions are never the answer, even for a single offending file -- if the rule is genuinely wrong for one file, the rule is almost certainly wrong for the directory, and the directory is the right scope to record and review.
