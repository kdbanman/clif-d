...
- [ ] **Catastrophic failure only:** The test checks for a failure that would make the system fundamentally unusable, not a subtle bug. *(keep scope narrow)*
- [ ] **Fast:** The test completes in seconds. *(the whole suite should run in under 2 minutes)*
- [ ] **Simple:** The test is trivially easy to understand — no complex setup, no nuanced assertions. *(Principles §8, no logic in tests)*
